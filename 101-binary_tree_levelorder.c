 ...
 *
 *  Use the recursive helper function to get the
 * height of left and right subtrees of the current node recursively.
 *
 * Get the max of left height and right height and
 ...
 *  If the level is 1, print the node.
 * If the level is greater than 1, recursively
 ...
 *  Then, we iterate through the tree from level 1 to height.
 * For each level, we call print_level_order() function.
 *
 *  In print_level_order() function, we first check if the
 * current node is NULL. If yes, then we return.
 *
 * If the current node is not NULL, then we check if the
 * current level is 1. If yes, then we print the data of the
 * current node.
 *
 * If the current level is greater than 1,
 * then we recursively call print_level_order() for left and
 * right children of the current node.
 *
 * The recursion ends when the current level is 1.
 *
 * Time Complexity: O(n)
 */
